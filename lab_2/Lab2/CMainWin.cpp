...
// функция отображает рисунок в bmp в окне windows
...
{
	BITMAP BitMap;
	//определяем параметры загружаемого объекта (картинки)
	GetObject(hBit, sizeof(BITMAP), &BitMap);

	//получаем размеры для копирования
...
//область в окне, которая сохраняется
//имя файла
{
	...
	int BitToPixel = 16;								//цветовая глубина 16 бит
	HDC ImageDC = GetDC(hWnd);							
	HDC MemoryDC = CreateCompatibleDC(ImageDC);			
...
CMainWin::CMainWin()
{
	this->Create(0, (LPCTSTR)L"Lab 2", WS_OVERLAPPEDWINDOW, rectDefault, 0, (LPCTSTR)IDR_MAINFRAME);
}
